...
def generate_respondent(resp_id):
    ...
    # Q1: Brand Awareness (70% aware)
    q1_aware = random.choices([1, 0], weights=[70, 30])[0]
    
    if q1_aware == 0:
        # Not aware - skip all usage-related questions
        segment = 4  # Excluded
        return {
    ...
    # Q10: NPS Score (0-10, overall brand NPS)
    q10_nps = random.randint(max(0, q4_recommendation - 2), min(10, q4_recommendation + 2))
...
aware_count = sum(1 for r in respondents if r['Q1'] == 1)
print(f"\nSummary:")
print(f"  Brand Aware: {aware_count} ({aware_count/len(respondents)*100:.1f}%)")
print(f"  Not Aware: {len(respondents)-aware_count} ({(len(respondents)-aware_count)/len(respondents)*100:.1f}%)")
